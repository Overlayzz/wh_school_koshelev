...
BEGIN
    ...
    CASE
        ...
        WHEN EXISTS(SELECT 1 FROM shop.client c WHERE c.phone = _phone and c.client_id = _client_id) THEN
            RETURN public.errmessage(_errcode := 'shop.client_ins.phone_exists',
                                     _msg := 'Номер телефона уже принадлежит этому пользователю!',
                                     _detail := concat('phone = ', _phone));
        WHEN EXISTS(SELECT 1 FROM shop.client c WHERE c.phone = _phone) THEN
            RETURN public.errmessage(_errcode := 'shop.client_ins.phone_exists',
